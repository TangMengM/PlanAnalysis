...
namespace PlanAnalysis
{
    public partial class FrmBuilding : Form
    {
        ...
        public FrmBuilding(MyPluginEngine.IApplication hook)
        {
            InitializeComponent();
            this.hk = hook;
            //不显示最大化最小化按钮
        ...
        private void btOk_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请选择输入城规用地数据！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (txtInput2.Text.Equals(""))
            {
                MessageBox.Show("请选择输入土规用地数据！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            //更改数据源
            string defaultPath = System.IO.Directory.GetCurrentDirectory().ToString() + "\\Data\\PlanAnalysis\\城规土规冲突.mxd";//获取Debug路径
            string fileName;
        ...
        {
            txtInput1.Text = System.IO.Directory.GetCurrentDirectory().ToString() + "\\Data\\PlanAnalysis\\建设.shp";
            txtInput2.Text = System.IO.Directory.GetCurrentDirectory().ToString() + "\\Data\\PlanAnalysis\\耕地.shp";
            txtOutput.Text = System.IO.Directory.GetCurrentDirectory().ToString() + "\\Data\\PlanAnalysis\\01城规土规\\城规土规冲突.shp";
        }
    }
}
